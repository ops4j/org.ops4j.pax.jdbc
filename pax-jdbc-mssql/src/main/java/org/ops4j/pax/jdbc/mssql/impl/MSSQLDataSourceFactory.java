...
public class MSSQLDataSourceFactory implements DataSourceFactory {
	...
	public ConnectionPoolDataSource createConnectionPoolDataSource(Properties props) throws SQLException {
		try {
			ConnectionPoolDataSource cpds = ConnectionPoolDataSource.class.cast(mssqlConnectionPoolDataSourceClass.newInstance());
	...
	private void setProperties(Class<?> dataSourceClass, Object dataSourceInstance,Properties props) throws Exception {
		
		setProperty(props.getProperty(DataSourceFactory.JDBC_URL),dataSourceClass,dataSourceInstance,"setURL");
        setProperty(props.getProperty(DataSourceFactory.JDBC_DATABASE_NAME),dataSourceClass,dataSourceInstance,"setDatabaseName");
        setProperty(props.getProperty(DataSourceFactory.JDBC_SERVER_NAME),dataSourceClass,dataSourceInstance,"setServerName");
        setProperty(props.getProperty(DataSourceFactory.JDBC_PORT_NUMBER),dataSourceClass,dataSourceInstance,"setPortNumber");
        setProperty(props.getProperty(DataSourceFactory.JDBC_USER),dataSourceClass,dataSourceInstance,"setUser");
        setProperty(props.getProperty(DataSourceFactory.JDBC_PASSWORD),dataSourceClass,dataSourceInstance,"setPassword");
        
	}
	
	private void setProperty(String propertyValue, Class<?> dataSourceClass, Object dataSourceInstance, String dataSourceMethodName ) throws Exception {
		if(propertyValue != null) {
			dataSourceClass.getMethod(dataSourceMethodName, String.class).invoke(dataSourceInstance,propertyValue);
		}
	}
