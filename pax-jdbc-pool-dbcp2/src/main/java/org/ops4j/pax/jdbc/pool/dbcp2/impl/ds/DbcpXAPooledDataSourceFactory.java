...
public class DbcpXAPooledDataSourceFactory extends XAPooledDataSourceFactory {
   ...
   */
  public DbcpXAPooledDataSourceFactory(DataSourceFactory dsFactory,
  ...
  protected ObjectName getJmxName(String dsName) {
    ...
    try {
      ...
      throw new IllegalArgumentException("Invalid object name for data source"
          + dsName, e);
    }
  }
  ...
  protected Iterable<Object> internalCreateDatasource(Object ds) {
    DataSourceXAConnectionFactory connFactory = new DataSourceXAConnectionFactory(
        tm, (XADataSource) ds);
    PoolableManagedConnectionFactory pcf = new PoolableManagedConnectionFactory(
        connFactory, null);
  ...
  protected DataSource doStart(Iterable<Object> mds) throws Exception {
    ...
    PoolableManagedConnectionFactory pcf = (PoolableManagedConnectionFactory) it
        .next();
    DataSourceXAConnectionFactory connFactory = (DataSourceXAConnectionFactory) it
        .next();
    GenericObjectPool<PoolableConnection> pool = new GenericObjectPool<PoolableConnection>(
        pcf, conf);
    TransactionRegistry transactionRegistry = connFactory
        .getTransactionRegistry();
    return new CloseableManagedDataSource<PoolableConnection>(pool,
        transactionRegistry);
